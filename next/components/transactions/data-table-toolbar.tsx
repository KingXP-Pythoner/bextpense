...
export function DataTableToolbar<TData>({
		...
		filterableColumns.map((field) => {
			return {
				...
				localSelectedValues: new Set(
					(table
						.getColumn(field.id ? String(field.id) : "")
	...
	const isFilteredLocal =
		filterableColumnStates.filter((state) => state.localSelectedValues.size > 0)
			.length > 0 || !isInputEmpty;

	const _setDebouncedFilterValues = useDebouncedCallback(
	...
	return (
		<div
			className={cn(
				"flex bg-background w-full items-center p-1 justify-between gap-2 ",
				className,
			)}
			{...props}
		>
