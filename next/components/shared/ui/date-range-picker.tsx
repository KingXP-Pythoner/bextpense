...
export function DateRangePicker({
	...
	const [dateParams, setDateParams] = useQueryStates(
		{
			fromDate: parseAsTimestamp.withDefault(
				dayjs.utc(defaultDateRange?.fromDate).toDate(),
			),
			toDate: parseAsTimestamp.withDefault(
				dayjs.utc(defaultDateRange?.toDate).toDate(),
			),
		},
		{
			clearOnDefault: true,
	...
	const date = React.useMemo(() => {
		function parseDate(timestamp: number | null) {
			...
			return isNaN(parsedDate.toDate().valueOf())
				? undefined
				: parsedDate.toDate();
		}
		...
		return {
			from:
				parseDate(dayjs.utc(dateParams.fromDate).toDate().valueOf()) ??
				dayjs.utc(defaultDateRange?.fromDate).toDate(),
			to:
				parseDate(dayjs.utc(dateParams.toDate).toDate().valueOf()) ??
				dayjs.utc(defaultDateRange?.toDate).toDate(),
		};
							...
							{date?.from ? (
								date.to ? (
									<>
										...
										{dayjs.utc(date.to).format("MMM DD, YYYY")}
									</>
								) : (
									dayjs.utc(date.from).format("MMM DD, YYYY")
								)
					...
					<div className="flex">
						...
						<div>
							<Calendar
								...
								mode="range"
								defaultMonth={
									date?.from ? dayjs.utc(date.from).toDate() : undefined
								}
								selected={date}
