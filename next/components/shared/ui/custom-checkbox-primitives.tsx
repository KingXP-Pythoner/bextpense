...
const CustomCheckboxContext = React.createContext<CustomCheckboxContextValue | undefined>(undefined)

const useCustomCheckboxContext = () => {
	const context = React.useContext(CustomCheckboxContext)
	if (!context) {
		throw new Error("useCustomCheckbox must be used within a CustomCheckboxContext")
	}
	return context
}

const Provider = ({ children, ...props }: CustomCheckboxContextValue & { children: React.ReactNode }) => {
	const [checked, setChecked] = React.useState<CheckedState>(props.checked ?? "indeterminate")
...
const Root = ({ checked: checkedProp, onCheckedChange, ...props }: CustomCheckBoxProps) => {
