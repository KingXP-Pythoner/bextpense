...
export const fetchAllTransactions = async (searchParamsObject: TransactionsParsedSearchParams) => {
    return await unstable_cache(async () => {

        const parsedSearchParams = getAllTransactionsSearchParamsBuilder(searchParamsObject)


        const response = await fetch(`${process.env.NEXT_PUBLIC_DEV_MODE_FRONTEND_URL}${endpoints.bff.transactions.listAllTransactions}?${parsedSearchParams.toString()}`, {
        method: "GET",
        headers: {
