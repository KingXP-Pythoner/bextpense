...
export const toRecurringRevenueChart = (
	...
	yAxis: {
		...
		min: 0,
		max: data.recurringRevenue.monthlyData.reduce(
			(max, item) => Math.max(max, item.amount),
			0,
		),
		tickInterval: Math.ceil(
			data.recurringRevenue.monthlyData.reduce(
				(max, item) => Math.max(max, item.amount),
				0,
			) / 4,
		),
...
export const toRecurringExpensesChart = (
	data: TTransactionOverviewResponse,
): Options => ({
	...HIGHCHART_DEFAULT_OPTIONS,
	xAxis: {
	...
	yAxis: {
		...HIGHCHART_DEFAULT_OPTIONS.yAxis,
		min: 0,
		max: data.recurringExpenses.monthlyData.reduce(
			(max, item) => Math.max(max, item.amount),
			0,
		),
		tickInterval: Math.ceil(
			data.recurringExpenses.monthlyData.reduce(
				(max, item) => Math.max(max, item.amount),
				0,
			) / 4,
		),
...
export const toSavingsChart = (data: TTransactionOverviewResponse): Options => {
	const max = data.savings.monthlyData.reduce(
		(max, item) => Math.max(max, item.amount),
		0,
	);
	return {
...
export const toIncomeVsExpensesChart = (
	data: TTransactionOverviewResponse,
): Options => ({
	...HIGHCHART_DEFAULT_OPTIONS,
	xAxis: {
...
export const toTransactionSummaryChart = (
	...
	while (
		...
		currentDate.isSame(maxDate, "month")
	) {
	...
	const completeMonthlyData = allMonths.reduce(
		(acc, month) => {
	...
	return {
		...
		series: [
			...
			},
			{
				...
				data: allMonths.map(
					(month) =>
						completeMonthlyData[month].income -
						completeMonthlyData[month].expense,
				),
				color: "var(--color-chart-2)",
				zIndex: 1,
			},
