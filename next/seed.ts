...
for (let m = 0; m <= monthDiff; m++) {
	...
	for (let i = 0; i < businessTransactions; i++) {
		...
		const baseAmount =
			(baseIncome.business / businessTransactions) * growthMultiplier;
		transactions.push({
			id: faker.string.uuid(),
			userId: mockUserId,
	...
	for (let i = 0; i < monthlyExpenses; i++) {
		...
		if (["rent", "mortgage"].includes(category)) {
			...
			["property_taxes", "car_maintenance", "electronics"].includes(category)
		) {
			maxAmount = 1000;
...
try {
	...
	if (!fs.existsSync(basePath)) {
		throw new Error(
			"Base path does not exist. Ensure you have not modified the project structure already.",
		);
	}
	...
	console.log(
		`Generated ${transactionTypes.length} transaction types to transactionTypes.json`,
	);
	...
	console.log(
		`Generated ${transactions.length} transactions to transactions.json`,
	);
} catch (error) {
