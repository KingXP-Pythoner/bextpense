...
export const createTransactionBaseSchema = z.object({
	...
	transactionDate: z.number().refine((val) => !dayjs(val).isAfter(dayjs()), {
 ...
 */
export const transactionDtoSchema = createTransactionBaseSchema
	.omit({
...
export const transactionRequestSchema = z.object({
	...
	type: z
		.array(z.enum(TRANSACTION_TYPES as unknown as [string, ...string[]]))
		.optional()
		.default([]),
	category: z
		.array(
...
export const transactionFilterParamsSchema = z
	.object({
		...
		type: z.array(z.enum(TRANSACTION_TYPES)).optional().default([]),
		title: z.string().optional().default(""),
		search: z.string().optional().default(""),
		category: z
			.array(
				z.enum([
					...Object.values(TRANSACTION_CATEGORIES.income),
					...Object.values(TRANSACTION_CATEGORIES.expense),
				] as [string, ...string[]]),
			)
			.optional()
			.default([]),
		...
		fromDate: z
			.date()
			.optional()
			.default(dayjs.utc().startOf("month").subtract(1, "month").toDate()),
		...
		pageSize: z.coerce
			...
			.transform((amount) =>
				Math.min(
					DEFAULT_PER_PAGE_MAX,
					Math.max(DEFAULT_PER_PAGE_MIN, Math.round(amount / 10) * 10),
				),
			),
	})
	.strict();
 ...
 */
export type TransactionFilterParams = z.infer<
	typeof transactionFilterParamsSchema
>;
