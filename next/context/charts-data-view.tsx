...
export const useChartsDataView = () => {
	...
	if (!context) {
		throw new Error(
			"useChartsDataView must be used within a ChartsDataViewProvider",
		);
	}
	return context;
};
...
const calculateMetrics = (
	data: TTransactionOverviewResponse["recurringRevenue"]["monthlyData"],
) => {
...
export const TransactionOverviewProvider = ({
	children,
	initialData,
}: { children: ReactNode; initialData: TTransactionOverviewResponse }) => {
	...
	const filteredData = useMemo(() => {
		...
		const filteredRevenueData = filterDataByTimeRange(
			initialData.recurringRevenue.monthlyData,
			timeRange,
		);
		const filteredExpensesData = filterDataByTimeRange(
			initialData.recurringExpenses.monthlyData,
			timeRange,
		);
		const filteredSavingsData = filterDataByTimeRange(
			initialData.savings.monthlyData,
			timeRange,
		);
	...
	return (
		<ChartsDataViewContext.Provider
			value={{ timeRange, setTimeRange, chartData: filteredData }}
		>
			{children}
		</ChartsDataViewContext.Provider>
	);
