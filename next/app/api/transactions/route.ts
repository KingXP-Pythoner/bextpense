...
import { getAllTransactionsSearchParamsBuilder, transactionSearchParamsCache, MutableTransactionSearchParams } from "@/components/transactions/search-params-parser";
import { z } from "zod";
import {
...
const fetchTransactions = async (validatedInput: TransactionRequestParams) => {
  const settled = await tryCatchAsync(async () => {
    const searchParams = getAllTransactionsSearchParamsBuilder(validatedInput as any as MutableTransactionSearchParams)
    console.log("SEARCH-PARAMS-ROUTE-FETCH (final params being sent to API):", searchParams.toString())
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("SEARCH-PARAMS-OBJECT-ROUTE (as object):", JSON.stringify(searchParamsObject, null, 2))
    
    // First parsing using nuqs (converts strings to proper types)
    const parsedSearchParams = transactionSearchParamsCache.parse(searchParamsObject)
    console.log("PARSED-SEARCH-PARAMS-ROUTE (after nuqs parsing):", JSON.stringify(parsedSearchParams, null, 2))
    ...
    const mutableParams: MutableTransactionSearchParams = {
      ...
      pageIndex: parsedSearchParams.pageIndex || parseInt(searchParamsObject.pageIndex, 10) || 1,
      pageSize: parsedSearchParams.pageSize || parseInt(searchParamsObject.pageSize, 10) || 10,
    ...
    const processedParams = {
      ...validatedParams,
      fromDate: validatedParams.fromDate || dayjs.utc().startOf("month").subtract(1, "month").toDate(),
